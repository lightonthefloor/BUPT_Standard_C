...
uint8_t Usart_Rx_Data[2][SBUS_RX_BUF_NUM];
...
void USART1_Rx_Init() {
	USART1_Receive_init(Usart_Rx_Data[0], Usart_Rx_Data[1], SBUS_RX_BUF_NUM);
}
...
void USART1_IRQHandler(void) {
	if (huart1.Instance->SR & UART_FLAG_RXNE)//接收到数据
	{
		...
		if ((hdma_usart1_rx.Instance->CR & DMA_SxCR_CT) == RESET) {
			...
			if (this_time_rx_len){
				Received_Data_Dealer(Usart_Rx_Data[0]);
			}
		} else {
			...
			if (this_time_rx_len){
				Received_Data_Dealer(Usart_Rx_Data[1]);
			}
		}
	}
}